...
<section>
  ...
  <div className="box-a  w-[50%]">
  {data.slice(0,5).map(a=>(
    <>
 <div className='box-a p-3 flex '>
   <img className='h-[50px] object-cover' src={a.featured_image_src} alt="" />
 <h1 className='text-1 text-1xl text-red-800 font-bold p-2'> <Link href={`/details/${a.id}`}>{a.title.rendered}</Link></h1>
 </div>
   </>
))}
</div>
